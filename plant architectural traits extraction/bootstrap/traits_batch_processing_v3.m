...
% 文件夹路径
folder_path = 'E:\Tomato_data\seg_organs\instances_and_traits_20230315_p2\';
...
for N = 1:144
    ...
    csv_file_path = fullfile(folder_path, [num2str(N) '_filtered_traits_no_empty_v3.csv']);
    ...
    for i = 1:size(filtered_data, 1)
        if filtered_data(i, 1) == 3
            % 找到第三列及以后大于等于0.01 0.015 0.02 0.03的值，将其改为0
            values_to_change = filtered_data(i, 3:end) >= 0.01;
            filtered_data(i, 2 + find(values_to_change)) = 0;
    
            % 计算第三列及以后的数据的最大值
            max_value = max(filtered_data(i, 3:end));
    
            % 将第三列及以后的数据改为0
            filtered_data(i, 3:end) = 0;
    
            % 将第三列的值改为最大值
            filtered_data(i, 3) = max_value;
        end
    end
