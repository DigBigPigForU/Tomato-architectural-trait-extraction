...
DataSize = 50;
times = 1;      % range between [-sigma*,sigma].*times
FolderNum = 5;
    ...
    for folderIndex = 1:length(foldersPrefixes)
        ...
        while numel(currentColumn) < DataSize
            ...
            if ~isempty(files)
                ...
                % 随机选择一个第一列为0的行
                rowsWith3 = find(data(:, 1) == 1);
                if ~isempty(rowsWith3)
                    selectedRow = rowsWith3(randi(length(rowsWith3)));
                
                    % 将该行第三列及以后的非零非空相邻值做差，将结果的平均值放在容器中的当前列
                    valuesInRow = data(selectedRow, 3:end);  % 获取第三列及以后的所有值
                    nonZeroValues = valuesInRow(valuesInRow ~= 0);  % 去除零值
                    
                    if numel(nonZeroValues) > 1
                        differences = diff(nonZeroValues);  % 计算相邻值的差值
                        averageDifference = mean(differences);  % 计算差值的平均值
                        currentColumn = [currentColumn, averageDifference];
