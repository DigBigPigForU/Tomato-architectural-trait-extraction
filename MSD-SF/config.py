...
# Dataset config
"""Dataset Path"""
C.dataset_name = 'upload_to_google_colab_dataset'
C.dataset_path = osp.join(C.root_dir, 'datasets', 'upload_to_google_colab_dataset')
...
C.x_root_folder = osp.join(C.dataset_path, 'DFolder')
C.x_format = '.png'
C.x_is_single_channel = True # True for raw depth, thermal and aolp/dolp(not aolp/dolp tri) input
C.ms_root_folder = osp.join(C.dataset_path, 'MSFolder')
C.ms_format = '.png'
C.ms_is_single_channel = False # True for raw depth, thermal and aolp/dolp(not aolp/dolp tri) input
...
C.num_eval_imgs = 5
C.num_classes = 4
C.class_names =  ['stems','fruits','leaves','flowers']
